...
This script optimizes the following segmentation hyperparameters:
- spatial_weight_x     (Real)
- spatial_weight_y     (Real)
- contour_weight       (Real)
- min_pixels           (Integer)
- top_components       (Integer)

It evaluates performance with a trained model and reports both sequence-level
...
def segment_characters_custom(image_path, min_pixels=3, top_components=5,
    ...
    # KMeans clustering with spatial + contour weights
    ...
    # Remove noise & pick top components
...
def evaluate_with_params(model, img_paths, device,
    ...
    for img_path in tqdm(
            img_paths,
            desc=f"Evaluating swx={swx:.2f}, swy={swy:.2f}, cw={cw:.1f}, mp={min_pixels}, tc={top_components}",
            leave=False):
...
def bayesian_optimize(model, img_paths, device, output_csv):
    ...
    # 5-D search space (3 real + 2 discrete int)
    space = [
        Real(0.1, 10.0, name='spatial_weight_x'),
        Real(0.1, 2.0, name='spatial_weight_y'),
        Real(20, 400, name='contour_weight'),
        Integer(1, 10, name='min_pixels'),
        Integer(1, 10, name='top_components')
    ]
    ...
    def objective(spatial_weight_x, spatial_weight_y, contour_weight,
        ...
        print(f"\n[Step {call_counter['n']}/30] Testing params:"
              f" swx={spatial_weight_x:.2f}, swy={spatial_weight_y:.2f}, "
              f"cw={contour_weight:.1f}, mp={min_pixels}, tc={top_components}")

        sampled_paths = bootstrap_sample(img_paths, n_samples=2000)

        seq_acc, char_acc, total = evaluate_with_params(
            model, sampled_paths, device,
            spatial_weight_x, spatial_weight_y, contour_weight,
            min_pixels, top_components
        )
    ...
    print("\nBest Params:")
    print(f"  spatial_weight_x = {best[0]:.3f}")
    print(f"  spatial_weight_y = {best[1]:.3f}")
    print(f"  contour_weight   = {best[2]:.3f}")
    print(f"  min_pixels       = {best[3]}")
    print(f"  top_components   = {best[4]}")
    print(f"\nBest Sequence Accuracy: {-opt.fun:.2f}%")
